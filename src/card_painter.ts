import { cardImageNames, Color, defaultEmoji, imageNames, Item, Modifier, Type, ValueSign } from "./states"
...
export function updateCanvas(
  ...
  function placeSecondary(inset: number) {
    if (primaryActionType == Type.DEFENSE || primaryActionType == Type.DEFENSE_SKILL) {
      ...
      addSecondaryValue(context, (secondaryMovementValue + movementBonus).toString(), 143, inset + 121, movementBonus)
    } else if (primaryActionType == Type.MOVEMENT) {
      addImage(context, "defense", 70, inset)
      addSecondaryValue(context, (secondaryDefenseValue + defenseBonus).toString(), 143, inset + 121, defenseBonus)
    } else {
      addImage(context, "movement", 64, inset)
      addImage(context, "defense", 70, inset - 209)
      addSecondaryValue(context, (secondaryMovementValue + movementBonus).toString(), 143, inset + 121, movementBonus)
      addSecondaryValue(context, (secondaryDefenseValue + defenseBonus).toString(), 143, inset - 79, defenseBonus)
    }
  }
  ...
  if (color != Color.PURPLE) {
    ...
    addInitiative(context, (initiative + initiativeBonus).toString(), 143, 192, initiativeBonus)
  }
  ...
  switch (modifier) {
    case Modifier.AREA:
      ...
      modifierValueWidth = addPrimaryValue(context, (modifierValue + areaBonus).toString(), 1052, primaryActionHeight + 82, areaBonus)
      break
    case Modifier.RANGE:
      addImage(context, `range_${lowerColor}`, 936, primaryActionHeight - 77)
      modifierValueWidth = addPrimaryValue(context, (modifierValue + rangeBonus).toString(), 1052, primaryActionHeight + 82, rangeBonus)
      break
  }
  if ((modifier == Modifier.AREA || modifier == Modifier.RANGE) && (modifierValueSign == ValueSign.PLUS || modifierValueSign == ValueSign.MINUS)) {
  ...
  if (color == Color.PURPLE) {
    ...
    switch (primaryActionType) {
      ...
      case Type.ATTACK:
        ...
        if (primaryActionValueSign != ValueSign.EXCLAMATION)
          primaryValueWidth = addPrimaryValue(context, (primaryActionValue + attackBonus).toString(), 142, primaryActionHeight + 82, attackBonus)
        break
      case Type.MOVEMENT:
        addImage(context, `movement_${lowerColor}`, 43, primaryActionHeight - 68)
        if (primaryActionValueSign != ValueSign.EXCLAMATION)
          primaryValueWidth = addPrimaryValue(context, (primaryActionValue + movementBonus).toString(), 142, primaryActionHeight + 82, movementBonus)
        break
      case Type.DEFENSE:
        addImage(context, `defense_${lowerColor}`, 51, primaryActionHeight - 74)
        if (primaryActionValueSign != ValueSign.EXCLAMATION)
          primaryValueWidth = addPrimaryValue(context, (primaryActionValue + defenseBonus).toString(), 142, primaryActionHeight + 82, defenseBonus)
        break
      case Type.DEFENSE_SKILL:
    ...
    if (primaryActionType != Type.SKILL && primaryActionValueSign != ValueSign.NONE) {
      if (primaryActionValueSign == ValueSign.EXCLAMATION)
        addPrimaryValue(context, "!", 142, primaryActionHeight + 82)
      else
...
function addInitiative(context: CanvasRenderingContext2D, text: string, x: number, y: number, bonus: number) {
  addSquishedOutlinedText(context, text, x, y, 197, 14, 0.92, false, bonus)
}

function addPrimaryValue(context: CanvasRenderingContext2D, text: string, x: number, y: number, bonus: number = 0): number {
  return addSquishedOutlinedText(context, text, x, y, 156, 14, 0.875, false, bonus)
}
...
function addSecondaryValue(context: CanvasRenderingContext2D, text: string, x: number, y: number, bonus: number = 0) {
  addSquishedOutlinedText(context, text, x, y, 136, 14, 0.875, false, bonus)
}
